...
async function buildFeature(
  ...
  const iconCaption = buildCaption(
    personName,
    typeOfChecking,
    address.description,
  );
...
export async function convertRoutesToGeojson(
  routes: Routes,
  getCoordinates: GetCoordinates,
...
if (import.meta.vitest != null) {
  test.each([
    ...
    "convertRoutesToGeojson",
    (personName, typeOfChecking, addressDescription, result) => {
      expect(
        buildCaption(personName, typeOfChecking, addressDescription),
      ).toEqual(result);
    },
  );
}
